        ...
        steps {
            ...
            if (env.GIT_BRANCH == 'origin/development') {
                withKubeConfig([credentialsId: 'cred-km-aks-lab01'])
                {
                    ...
                    sh 'cat kube/service.yaml | sed "s/{{NODE_PORT}}/nodePort: 30005/g" | kubectl apply -f - -n development'
                }
            }
            else if (env.GIT_BRANCH == 'origin/master') {
                withKubeConfig([credentialsId: 'cred-km-aks-lab01'])
                {
                    ...
                    sh 'cat kube/service.yaml | sed "s/{{NODE_PORT}}/nodePort: 32005/g" | kubectl apply -f - -n production'
                }
            }
            else{}
            }
